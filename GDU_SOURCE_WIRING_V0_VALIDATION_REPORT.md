...
状态：接线与真实 Pilot 测试通过；已纳入 Builder v0 冻结基线。
...
因此当时 84 个唯一测试均已在具备各自依赖的环境中通过。

后续冻结前审计已统一到 Conda `gdu`（Python 3.12.13），全项目 95 项测试一次性全部通过，0 跳过；同一 Pilot 03 配置重复运行的三文件包逐字节一致。
