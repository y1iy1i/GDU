...
状态：机械格式最终通过，但扩大页包后的章节边界任务未通过；已转为对照模型。
...
- 输入扩大为物理页 1、8–12；
- 返回 4 个对象，Adapter response Schema、GDU 字段 Schema、原文逐字接地仍全部通过；
- 本地结果：`tmp/remote_cp1_qwen37/boundary_8_12.json`（Git 忽略）；
- 结果 SHA-256：`1a41dbb27907bdf737b53511165197db24f01ff64f7030733e455ba479e37246`；
- 模型仍只引用第 1、8 页，把第二节范围保留为 8–8，没有利用第 9–12 页判断章节终点。

这说明“机械合法”和“完成理解任务”是两道不同的门。Qwen 通过了格式门，但没有通过本次章节边界语义门。依据用户的模型选择规则，后续正式实验改用 `deepseek-v4-flash-0731`，本报告保留作对照。
