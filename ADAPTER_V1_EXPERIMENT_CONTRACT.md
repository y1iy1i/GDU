...
大白话：Builder 不直接相信模型写的一大段话。它只把当前允许看的原文片段和已公开的工作结果交给 Adapter，Adapter 必须交回可验证的 JSON。
...
当前首个远程候选为阿里云百炼 Token Plan 的 `qwen3.7-plus`，配置见 `configs/api/aliyun-token-plan-qwen3.7-plus.example.json`。该模型支持原生 JSON Mode，返回内容仍由本地 Schema 严格校验；配置文件本身不会触发请求。
